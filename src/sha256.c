/* sha256.c - small SHA-256 implementation (public-domain-style) */
#include "sha256.h"
#include <string.h>
#include <stdlib.h>
#include <stdio.h>
...
void sha256(const void *data, size_t len, uint8_t digest[32]) {
    ...
    /* append 64-bit big-endian length (we'll support up to 2^64-1 bits) */
...
/* Print SHA-256 of a file given as first argument.
   Usage: ./sha256sum FILE
   Exits 0 on success, nonzero on error. */
int main(int argc, char **argv) {
    if (argc != 2) {
        fprintf(stderr, "Usage: %s FILE\n", argc > 0 ? argv[0] : "sha256sum");
        return 2;
    }
    ...
    if (filesize > 0) {
        ...
        while (read_total < filesize) {
            size_t n = fread(data + read_total, 1, filesize - read_total, f);
            if (n == 0) {
                if (ferror(f)) {
